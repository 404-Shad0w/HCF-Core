...
final class Ffa extends Timer {

	/** @var string[] */
	private array $commands = [];

	public function __construct() {
		parent::__construct('Ffa', 'Use timer to ffa', '&cFfa end in&r&7:', 60 * 60);
	}
		...
		if ($enabled) {
			HCF::getInstance()->getServer()->getCommandMap()->register('HCF', new class($this) extends Command {
				...
				public function __construct(
					...
					parent::__construct('ffa', 'Use command to ffa');
				...
				public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
					...
					switch (strtolower($args[0])) {						
						case 'add':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /ffa add [command]'));
								return;
							}
							$command = $args[1];
						...
						case 'remove':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /ffa remove [position]'));
								return;
							}
							$position = (int) $args[1];
			...
			$command = HCF::getInstance()->getServer()->getCommandMap()->getCommand('ffa');
