...
final class PackageAll extends Timer {

	/** @var string[] */
	private array $commands = [];

	public function __construct() {
		parent::__construct('PackageAll', 'Use timer to Partner Package All', '&dPkg All&r&7:', 60 * 60);
	}
	...
	public function setEnabled(bool $enabled) : void {
		...
		if ($enabled) {
			HCF::getInstance()->getServer()->getCommandMap()->register('HCF', new class($this) extends Command {
				...
				public function __construct(
					private PackageAll $manager
				) {
					parent::__construct('packageall', 'Use command to partner package all');
					$this->setPermission('package_all.command');
				}
				...
				public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
					...
					if (count($args) < 1) {
						$sender->sendMessage(TextFormat::colorize('&cUse /packageall help'));
						return;
					}
					...
					switch (strtolower($args[0])) {
						case 'help':
							$text = [
								'&l&4Package All Commands&r',
								'&4/packageall add &7- Use command to add command',
								'&4/packageall remove &7- Use command to remove command',
								'&4/packageall list &7- Use command to view list'
							];
						...
						case 'add':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /packageall add [command]'));
								return;
							}
							$command = $args[1];
							...
							$sender->sendMessage(TextFormat::colorize('&aYou have been added the command to partner package all.'));
							break;

						case 'remove':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /packageall remove [position]'));
								return;
							}
							$position = (int) $args[1];
			...
			$command = HCF::getInstance()->getServer()->getCommandMap()->getCommand('packageall');
